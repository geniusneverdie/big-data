...
def read_data(file_path):
    ...
    # 移除度小于5的节点
    for node, degree in list(node_degree.items()):
        if degree < 5:
...
def page_rank_reverse(graph, all_nodes, teleport_parameter = 0.85):
    ...
    for out_node, in_nodes in graph.items():
        for in_node in in_nodes:
            if in_node in node_idx and out_node in node_idx:  # 检查节点是否存在
...
def main():
    ...
    write_result('results/reverse_result.txt', top_100_nodes)
